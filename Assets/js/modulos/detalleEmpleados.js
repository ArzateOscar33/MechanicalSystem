...
document.querySelector('button[data-bs-target="#detalleEmpleados"]').addEventListener("click", function () {
    frmDetalle.reset();
    ...
    document.querySelector("#detalle_position_id").innerHTML = '<option value="">Seleccione</option>';
    limpiarHiddenInputs();
});
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    filtroDepartamento.addEventListener("change", function () {
        cargarEmpleadosPorDepartamento(this.value);
    });
    ...
    btnGenerarCredencial.addEventListener("click", function () {
        ...
        if (!id) {
            Swal.fire("Aviso", "Debe seleccionar un empleado para generar la identificación", "warning");
        } else {
...
function cargarEmpleadosPorDepartamento(idDepartamento) {
    ...
    http.onreadystatechange = function () {
        if (this.readyState === 4 && this.status === 200) {
            ...
            filtroEmpleado.innerHTML = '<option value="">Seleccione</option>';
            res.forEach(emp => {
                const option = new Option(`${emp.nombre_completo} (${emp.employee_number})`, emp.id);
    ...
    http.onreadystatechange = function () {
        if (this.readyState === 4 && this.status === 200) {
            ...
            if (data.id) {
                ...
                Swal.fire("Aviso", "Empleado no encontrado", "warning");
            }
        }
    };
}
...
function cargarDetalleEmpleado(id) {
    ...
    http.onreadystatechange = function () {
        if (this.readyState === 4 && this.status === 200) {
            const data = JSON.parse(this.responseText);
            if (data) {
                document.querySelector("#detalle_employee_number").value = data.employee_number;
                ...
                document.querySelector("#detalle_second_last_name").value = data.second_last_name;
                ...
                limpiarHiddenInputs();
                frmDetalle.insertAdjacentHTML("beforeend", `
...
function cargarPuestosDetalle(idDepartamento, idSeleccionado = null) {
    ...
    http.onreadystatechange = function () {
        if (this.readyState === 4 && this.status === 200) {
            ...
            res.forEach(puesto => {
                const option = new Option(puesto.name, puesto.id);
                if (puesto.id == idSeleccionado) option.selected = true;
...
function toggleEdicionEmpleado(editar) {
    const campos = document.querySelectorAll("#frmDetalleEmpleado input:not([type='hidden']), #frmDetalleEmpleado select");
    campos.forEach(el => {
...
function limpiarHiddenInputs() {
    const hiddenInputs = frmDetalle.querySelectorAll("input[type='hidden'][name='id'], input[type='hidden'][name='foto_actual']");
