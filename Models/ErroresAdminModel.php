...
class ErroresAdminModel extends Query
{
    ...
    public function obtenerCertificado($cert_number)
    {
        $sql = "SELECT 
                ...
                WHERE c.cert_number = ? AND r.status = 'pending'
                LIMIT 1";
    ...
    {
        $sql = "SELECT monitor_type, result FROM monitoring_results WHERE cert_number = ?";
        return $this->selectAll($sql, [$cert_number]);
    }

    public function obtenerValorActualCampo($cert_number, $campo)
    {
        $sql = "SELECT `$campo` FROM certificates WHERE cert_number = ?";
        $res = $this->select($sql, [$cert_number]);
        return $res ? $res[$campo] : null;
    }

    public function actualizarCampoCertificado($cert_number, $campo, $nuevoValor)
    {
        $sql = "UPDATE certificates SET `$campo` = ?, updated_at = NOW() WHERE cert_number = ?";
        return $this->save($sql, [$nuevoValor, $cert_number]);
    }
    ...
    public function actualizarEstadoSolicitud($correction_id, $user_id)
    {
        $sql = "UPDATE correction_requests 
                SET status = 'corrected', reviewed_by = ?, reviewed_at = NOW(), updated_at = NOW()
                WHERE id = ?";
        return $this->save($sql, [$user_id, $correction_id]);
    }

    public function insertarInspector($nombre)
    {
        $sqlCheck = "SELECT id FROM inspectors WHERE name = ? LIMIT 1";
        $existe = $this->select($sqlCheck, [$nombre]);

        if ($existe) {
            return $existe['id'];
        }

        $sqlInsert = "INSERT INTO inspectors (name) VALUES (?)";
        return $this->insertar($sqlInsert, [$nombre]);
    }

    public function obtenerInspectorPorId($id)
    {
        $sql = "SELECT * FROM inspectors WHERE id = ? LIMIT 1";
        return $this->select($sql, [$id]);
    }

    public function obtenerDireccionPorId($id)
    {
        $sql = "SELECT `number`, `street`, `city`, `state`, `zip` FROM addresses WHERE id = ?";
        return $this->select($sql, [$id]);
    }
    ...
    {
        $sql = "UPDATE certificates SET address_id = ?, updated_at = NOW() WHERE cert_number = ?";
    ...
    public function eliminar($id_error)
    {
        ...
        return $this->save($sql, ['rejected', $id_error]);
    }
}
