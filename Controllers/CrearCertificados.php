...
class CrearCertificados extends Controller
{
    ...
    private function generarCertificadoPDF($cert_number, $pdf_path, $data, $address_id)
    {
        ...
        <div style="text-align:center"><img src="' . $logo . '" height="80"></div>
        <div class="section"><b>Dirección:</b> {$direccion_texto}</div>
    
        <div class="section"><b>Cert Number:</b> ' . $cert_number . '</div>
    
        <div class="section"><b>Vehicle Information</b><br>
            <table>
                <tr><td class="label">VIN:</td><td>' . $data['vin'] . '</td></tr>
                <tr><td class="label">Year:</td><td>' . $data['year'] . '</td></tr>
                <tr><td class="label">Mfg In:</td><td>' . $data['fabricado_en'] . '</td></tr>
                <tr><td class="label">Make:</td><td>' . $data['marca'] . '</td></tr>
                <tr><td class="label">Model:</td><td>' . $data['modelo'] . '</td></tr>
                <tr><td class="label">License Plate:</td><td>' . $data['placa'] . '</td></tr>
                <tr><td class="label">Odometer:</td><td>' . $data['odometro'] . '</td></tr>
                <tr><td class="label">Owner\'s Name:</td><td>' . $data['propietario'] . '</td></tr>
            </table>
        </div>
    
        <div class="section"><b>Monitoring & Certificate</b><br>
            <table>
                <tr><td class="label">Misfire Monitor:</td><td>' . $data['monitor_fallo_encendido'] . '</td></tr>
                <tr><td class="label">Fuel System Monitor:</td><td>' . $data['monitor_sistema_combustible'] . '</td></tr>
                <tr><td class="label">Comprehensive Monitor Catalyst:</td><td>' . $data['monitor_integral_catalizador'] . '</td></tr>
                <tr><td class="label">Catalyst Monitor:</td><td>' . $data['monitor_catalizador'] . '</td></tr>
                <tr><td class="label">O2 Sensor Monitor:</td><td>' . $data['monitor_sensor_c2'] . '</td></tr>
                <tr><td class="label">Overall Test Result:</td><td>' . $data['resultado_prueba'] . '</td></tr>
            </table>
        </div>
    
        <div class="section">
            <b>Inspector:</b> ' . $data['inspector'] . '<br>
            <b>Inspector Signature:</b> ' . $data['firma_inspector'] . '<br>
            <b>EBITN:</b> ' . $data['ebitn'] . '<br>
            <b>Date:</b> ' . $data['fecha'] . '<br>
            <b>Expires:</b> ' . $data['fecha_expiracion'] . '<br>
        </div>
    
        <div class="section">
            <b>Geo Location:</b> ' . $data['latitud'] . ', ' . $data['longitud'] . '<br>
        </div>
    
        <div class="section">
            <b>Cert QR:</b><br>
            <img src="uploads/temp/' . $cert_number . '_qr.png" width="150">
        </div>
    ...
    $qrOutput = new QRCode($options);
    $qrOutput->render($contenidoQR, "uploads/temp/{$cert_number}_qr.png");
